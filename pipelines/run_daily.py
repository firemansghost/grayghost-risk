...
def parse_number_token(tok: str):
    ...
    if tok in {"-", "–", "—", ""}:  # handle various dashes/empty
        return None
...
def fetch_bybit_premium_now_pct():
    """Return current premium % from Bybit (linear BTCUSDT)."""
    try:
...
def fetch_binance_premium_now_and_7d_pct():
    """Return (current_premium_pct, 7d_avg_premium_pct).
       Tries Binance; if 'now' missing, falls back to OKX then Bybit."""
    now_pct = None
    try:
        now = http_json("https://fapi.binance.com/fapi/v1/premiumIndex?symbol=BTCUSDT", timeout=20)
    ...
    avg_pct = None
    try:
        arr = http_json("https://fapi.binance.com/fapi/v1/premiumIndexKlines?symbol=BTCUSDT&interval=1h&limit=168",
                        timeout=20)
    ...
    if now_pct is None:
        okx_now = fetch_okx_premium_now_pct()
        now_pct = okx_now if okx_now is not None else fetch_bybit_premium_now_pct()
...
def compute_term_structure_driver():
    ...
    # premium: Binance (with OKX/Bybit 'now' fallback inside)
    prem_now, prem_7d = fetch_binance_premium_now_and_7d_pct()

    parts = []
    if fann is not None:
        parts.append(sigmoid((fann - 10.0) / 10.0))     # ~10% ann funding feels neutral
